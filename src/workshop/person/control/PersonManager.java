...
public class PersonManager {
	public static void main(String[] args) {
		...
		PersonEntity[] persons = new PersonEntity[10];	//person을 여러명 담을 때, 자바는 배열 선언 시 내가 몇개 할지 줘야함 불편해서 컬랙션이라는 것을 사용
		//Person 객체생성 -> PersonEntity person = new PersonEntity();
	...
	public int findByGender(PersonEntity[] persons, char gender) {
		int genderCnt = 0;
		for(PersonEntity person : persons) {
			//char 타입은 primitive 타입으로 값을 비교할 때 == 연산자 사용해도 됨
	...
	public void showPerson(PersonEntity[] persons, String name) {
		for(PersonEntity person : persons) {
			//String은 refernce타입이므로 값을 비교할 때 equal()메서드 사용해야 함
